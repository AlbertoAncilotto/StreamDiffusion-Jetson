...
def build_engine(
    ...
    if free_mem > 6 * GiB:
        ...
        max_workspace_size = free_mem - activation_carveout - 0.5*GiB
    else:
    ...
    if 'vae_encoder' in onnx_opt_path or 'vae_decoder' in onnx_opt_path:
        engine.build(
            onnx_opt_path,
            fp16=True,
...
def export_onnx(
    ...
    with torch.inference_mode(), torch.autocast("cuda"):
        ...
        #inputs = model_data.get_sample_input(2*opt_batch_size, opt_image_height, opt_image_width)
        torch.onnx.export(
            model,
            inputs,
            onnx_path,
    ...
    # print('WARNING: built ONNX with static axes')
    # print('Model specified with dynamic:', model_data.get_dynamic_axes())
    # time.sleep(3)
    del model
    gc.collect()
